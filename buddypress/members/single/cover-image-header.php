...
<?php if (!bp_is_user_messages() && !bp_is_user_settings() && !bp_is_user_notifications() && !bp_is_user_profile_edit() && !bp_is_user_change_avatar() && !bp_is_user_change_cover_image()) : ?>
    ...
    <div id="cover-image-container" class="<?php echo esc_attr($profile_header_layout_style . ' ' . $social_networks_urls_div_class . ' ' . $my_profile); ?> bb-cover-image-container">
        <div id="header-cover-image" class="<?php echo esc_attr('cover-' . $profile_cover_height . ' width-' . $profile_cover_width . $has_cover_image_position . $has_cover_image . $has_default_cover); ?>">
            ...
            if (!empty($cover_image_url)) {
            ?>
                <img class="header-cover-img" src="<?php echo esc_url($cover_image_url); ?>" <?php
                        ...
                        ?>
                            ...
                            <?php if (bp_displayed_user_id() == get_current_user_id()) : ?>
                                <div class="referral-status" id="referral-status">
                                    <?php
                                    $referral_data = get_referral_data(bp_displayed_user_id());
                                    $referrals_this_month = count_referrals_this_month($referral_data);
                                    $referrals_this_year = count_referrals_this_year($referral_data);
                                    ?>
                                    <div class="display_ref_status">
                                        <span>Referrals Sent</span>
                                        <span class="ref_month">This Month <span class="number"><?php echo esc_html($referrals_this_month); ?></span></span>
                                        <span class="ref_year">This Year <span class="number"><?php echo esc_html($referrals_this_year); ?></span></span>
                                    </div>
                                </div>
                            <?php endif; ?>

                            <?php if (bp_displayed_user_id() != get_current_user_id() && is_user_logged_in()) : ?>
                                <div class="bb-referral" id="bb-referral-btn">
                                    <button id="send-referral-btn" class="button">Send Referral</button>
                                </div>
                            <?php endif; ?>

                            <!-- Send Send Referral popup -->
                            <div id="referral-popup" class="bb-action-popup" style="display: none;">
                                <div class="modal-mask bb-white bbm-model-wrap">
                                    <div class="modal-wrapper">
                                        <div class="modal-container">
                                            <header class="bb-model-header">
                                                <h4>Send Referral</h4>
                                                <a class="bb-close-referral-popup bb-model-close-button" href="#">
                                                    <span class="bb-icon-l bb-icon-times"></span>
                                                </a>
                                            </header>
                                            <div class="bb-referral-content bb-action-popup-content">
                                                <form id="referral-form" method="post">
                                                    <input type="hidden" id="recipient_id" value="<?php echo bp_displayed_user_id(); ?>">

                                                    <?php
                                                    if ($_SERVER["REQUEST_METHOD"] == "POST") {
                                                        $name = $_POST["ref-name"];
                                                        $phone = $_POST["ref-phoneno"];
                                                        $email = $_POST["ref-email"];
                                                        $message = $_POST["ref-message"];
                                                    ...
                                                    ?>
                                                    <label for="name">Full Name of Referral</label>
                                                    <input type="text" id="name" name="ref-name" required>
                                                    <?php if (!empty($errors["ref-name"])) echo '<p class="error">' . esc_html($errors["ref-name"]) . '</p>'; ?>

                                                    <label for="phoneno">Phone Number for Referral</label>
                                                    <input type="tel" id="phoneno" name="ref-phoneno" required>
                                                    <?php if (!empty($errors["ref-phoneno"])) echo '<p class="error">' . esc_html($errors["ref-phoneno"]) . '</p>'; ?>

                                                    <label for="email">Email for Referral</label>
                                                    <input type="email" id="email" name="ref-email" required>
                                                    <?php if (!empty($errors["ref-email"])) echo '<p class="error">' . esc_html($errors["ref-email"]) . '</p>'; ?>

                                                    <label for="message">Message</label>
                                                    <textarea id="message" name="ref-message" required></textarea>
                                                    <?php if (!empty($errors["ref-message"])) echo '<p class="error">' . esc_html($errors["ref-message"]) . '</p>'; ?>

                                                    <label for="type_of_referral">Type Of Referral</label>
                                                    <select id="type_of_referral" name="type_of_referral" required>
                                                    ...
                                                    <button type="submit" id="send-referral">Send</button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
